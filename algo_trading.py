...
def graph_candlestick_volume_show(stock, existingData, MA1, MA2, start_lim, end_lim):
    ...
    quotes = [tuple(x) for x in existingData[['Date','Open','High','Low','Close']].values]
    volume = existingData.iloc[:,5]
    ...
    ax0.fill_between(dates[-SP:], rsi[-SP:], 70, where=(rsi[-SP:]>=70), facecolor=negCol, edgecolor=negCol)
    ax0.fill_between(dates[-SP:], rsi[-SP:], 30, where=(rsi[-SP:]<=30), facecolor=posCol, edgecolor=posCol)
    ...
    candlestick_ohlc(ax, quotes, width=0.75, colorup='#53C156', colordown='#ff1717');
    ...
    maLeg = plt.legend(loc=9, ncol=2, borderaxespad=0, fancybox=True, prop={'size':7})
    ...
    ax2.text(0.015, 0.95, 'PPO (12,26,9)', va='top', color='w', transform=ax2.transAxes)
    ax2.fill_between(dates, ppo-ppo_ema9, 0, alpha=0.5, facecolor=fill_col, edgecolor=fill_col)
    plt.gca().yaxis.set_major_locator(mticker.MaxNLocator(prune='upper'))
    ...
    ax3.text(0.015, 0.95, 'OBV (21)', va='top', color='w', transform=ax3.transAxes)
    plt.gca().yaxis.set_major_locator(mticker.MaxNLocator(prune='upper'))
...
def graph_data_norsk_show(stock, existingData, MA1, MA2, start_lim, end_lim):
    ...
    ax2.fill_between(dates, macd-ema9, 0, alpha=0.5, facecolor=fill_col, edgecolor=fill_col)
    plt.gca().yaxis.set_major_locator(mticker.MaxNLocator(prune='upper'))
    ax2.set_facecolor('#07000d')
    plt.xlim(left=start_lim, right=end_lim)
    ax2.spines['bottom'].set_color('#5998ff')
    ...
    ax3.text(0.015, 0.95, 'MACD (12,26,9)', va='top', color='w', transform=ax2.transAxes)
    ax3.fill_between(dates, ppo-ema9, 0, alpha=0.5, facecolor=fill_col, edgecolor=fill_col)
    plt.gca().yaxis.set_major_locator(mticker.MaxNLocator(prune='upper'))
    ax3.set_facecolor('#07000d')
    plt.xlim(left=start_lim, right=end_lim)
    ax3.spines['bottom'].set_color('#5998ff')
...
def graph_data_show(stock, MA1, MA2, start_lim, end_lim):
    ...
    if existingData.shape[0] > 4:
        graph_candlestick_volume_show(stock, existingData, MA1, MA2, start_lim, end_lim)
    else:
        graph_data_norsk_show(stock, existingData, MA1, MA2, start_lim, end_lim)

def browse_stocks(stocks):
    stock_data = pd.DataFrame([[0, 0, 0, 0, 0, 0, 0]], columns=['Stock', 'Price','RSI', 'MACD', 'abs(MACD - EMA9)', 'MACD norm', 'RSI mean change'])
        ...
        try:
            ...
            for i in range(len(existingData.iloc[:,1])):
                ...
                try:
                    ...
                    emaslow, emaslow, macd = macd_calc(existingData.iloc[1:i,1])
                    min_macd = min(macd)
                    max_macd = max(macd)
                    if macd[-1] >= 0:
                    ...
                    macd_mean_change = np.sum(np.diff(macd[-n_days:-1]))/(n_day)
                    macd_mean_change = '{:.6f}'.format(macd_mean_change)
                except Exception as e:
                    pass
            price = existingData.iloc[-1,1]
            temp_pd = pd.DataFrame([[stock, price, rsi[-1], macd[-1], abs(macd[-1]-ema9[-1]), macd_norm, rsi_mean_change]], columns=['Stock', 'Price', 'RSI', 'MACD', 'abs(MACD - EMA9)', 'MACD norm', 'RSI mean change'])
...
def plot_RSI_change(MA1, MA2, start_lim, end_lim, num_stocks):
    ...
    stock_data_macd_ema9 = stock_data.sort_values(by='RSI mean change', ascending=False)
