...
def macd_calc(x, slow=26, fast=12):
    ...
    signal_line = 9ema of the macd line
...
def klinger_oscillator(stock, slow=55, fast = 34, period=13):
    hlc = (stock["High"] + stock["Low"] + stock["Close"])/3
    temp_trend = (hlc[-1] - hlc[-2])/hlc[-2]
    if (temp_trend > 0):
        trend = 1
    else:
        trend = -1

    dm = stock["High"] - stock["Low"]
    cm = dm.sum()
